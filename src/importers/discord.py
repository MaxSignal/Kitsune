...
from src.lib.artist import index_discord_channel_server, is_artist_dnp
from src.lib.autoimport import (
    encrypt_and_save_session_for_auto_import,
    kill_key
)
from src.lib.post import discord_post_exists
...
def process_channel(channel_id, server_id, import_id, key, before=None):  # noqa C901
    log(import_id, f'Starting importing {channel_id}', to_client=True)

    try:
        scraper = create_scrapper_session().get('https://discordapp.com/api/v6/channels/' + channel_id + '/messages?limit=50' + (('&before=' + before) if before != None else ''), headers={'authorization': key, 'user-agent': userAgent}, proxies=get_proxy())  # noqa E711
    ...
    while True:
        for post in scraper_data:
            try:
                ...
                if('attachments' in post and len(post['attachments']) > 0):
                    for attachment in post['attachments']:
                        ...
                        reported_filename, hash_filename, _ = download_file(
                            attachment['url'] if 'url' in attachment and attachment['url'] != None else attachment['proxy_url'],  # noqa E711
                            None,
                            None,
                            None,
                            name=filename,
                ...
                if (ENV_VARS.BAN_URL):
                    requests.request('BAN', f"{ENV_VARS.BAN_URL}/discord/server/{post_model['server']}")
                    requests.request('BAN', f"{ENV_VARS.BAN_URL}/api/discord/channel/{post_model['channel']}")
                    requests.request('BAN', f"{ENV_VARS.BAN_URL}/api/discord/channels/lookup?q={post_model['server']}")
...
def import_posts(import_id, key, channel_ids_str, contributor_id, allowed_to_auto_import, key_id):
    ...
    if len(channel_ids) > 0:
        ...
        log(import_id, "No channels has been supplied. No posts will be imported.", to_client=True)
