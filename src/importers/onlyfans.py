...
def import_posts(import_id, key, contributor_id, allowed_to_auto_import, key_id):  # noqa C901
    ...
    try:
        scraper = create_scrapper_session(useCloudscraper=False).get(
            'https://onlyfans.com/api2/v2/subscriptions/subscribes?offset=0&type=active&sort=desc&field=expire_date&limit=50',
            # @REVIEW: no reason to pass `headers` kwarg
            # if you pretty much construct the whole URL
            headers=get_auth_headers('https://onlyfans.com/api2/v2/subscriptions/subscribes?offset=0&type=active&sort=desc&field=expire_date&limit=50'),
    ...
    if len(scraper_data) == 0:
        log(import_id, "No active subscriptions or invalid key. No posts will be imported.", to_client=True)
    
    for subscription in scraper_data:
        user_id = subscription['username']
        internal_user_id = subscription['id']
        ...
        try:
            scraper = create_scrapper_session(useCloudscraper=False).get(
                f'https://onlyfans.com/api2/v2/users/{internal_user_id}/posts?limit=10&offset=0&skip_users_dups=1&format=infinite',
                # @REVIEW: no reason to pass `headers` kwarg
                # if you pretty much construct the whole URL
                headers=get_auth_headers(f'https://onlyfans.com/api2/v2/users/{internal_user_id}/posts?limit=10&offset=0&skip_users_dups=1&format=infinite'),
                cookies=auth_cookies,
                proxies=get_proxy()
            )
            scraper_data = scraper.json()
        ...
        while True:
            ...
            if scraper_data.get('hasMore'):
                ...
                try:
                    scraper = create_scrapper_session(useCloudscraper=False).get(
                        f'https://onlyfans.com/api2/v2/users/{internal_user_id}/posts?limit=10&offset={offset}&skip_users_dups=1&format=infinite',
                        headers=get_auth_headers(f'https://onlyfans.com/api2/v2/users/{internal_user_id}/posts?limit=10&offset={offset}&skip_users_dups=1&format=infinite'),
                        cookies=auth_cookies,
                        proxies=get_proxy()
                    )
                    scraper_data = scraper.json()
